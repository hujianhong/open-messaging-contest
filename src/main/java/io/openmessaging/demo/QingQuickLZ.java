...
public final class QingQuickLZ {
	...
	public static int compress(byte[] source,int len,byte[] destination, int level) {
		int src = 0;
...
//		byte[] d2;
		int fetch = 0;
		int last_matchstart = (len - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END - 1);
		int lits = 0;
		...
		while (src <= last_matchstart)
		{
			if ((cword_val & 1) == 1)
			{
				if (src > 3 * (len >> 2) && dst > src - (src >> 5))
				{
//					d2 = new byte[source.length+DEFAULT_HEADERLEN];
					write_header(destination, level, false, len, len + DEFAULT_HEADERLEN);
					System.arraycopy(source, 0, destination, DEFAULT_HEADERLEN, len);
//					return d2;
					return len + DEFAULT_HEADERLEN;
				}
			...
			if (level == 1)
			{
				...
				if (cache == 0 && hash_counter[hash] != 0 && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > 3 && source[src] == source[src - 3] && source[src] == source[src - 2] && source[src] == source[src - 1] && source[src] == source[src + 1] && source[src] == source[src + 2])))
				{
					...
					{
						int old_src = src;
						int remaining = ((len - UNCOMPRESSED_END - src + 1 - 1) > 255 ? 255 : (len - UNCOMPRESSED_END - src + 1 - 1));
		...
		write_header(destination, level, true, len, dst);
...
//		return d2;
	...
	static public int decompress(byte[] source,int len,byte[] destination)
	{
...
//			byte[] d2 = new byte[size];
			System.arraycopy(source, headerLen(source), destination, 0, size);
//			return d2;
			return size;
		}
