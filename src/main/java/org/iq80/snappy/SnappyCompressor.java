...
final class SnappyCompressor {
	...
	public static int compress(final byte[] uncompressed, final int uncompressedOffset, final int uncompressedLength,
			final byte[] compressed, final int compressedOffset) {
		// First write the uncompressed size to the output as a variable length
		// int
		...
		for (int read = 0; read < uncompressedLength; read += BLOCK_SIZE) {
			...
			compressedIndex = compressFragment(uncompressed, uncompressedOffset + read,
					Math.min(uncompressedLength - read, BLOCK_SIZE), compressed, compressedIndex, table);
		}
	...
	private static int compressFragment(final byte[] input, final int inputOffset, final int inputSize,
			final byte[] output, int outputIndex, final short[] table) {
		...
		// todo given that hashTableSize is required to be a power of 2, this is
		// overly complex
		...
		// Bytes in [nextEmitIndex, ipIndex) will be emitted as literal bytes.
		// Or
		...
		if (inputSize >= INPUT_MARGIN_BYTES) {
			...
			while (ipIndex <= ipLimit) {
				...
				// The body of this loop calls EmitLiteral once and then
				// EmitCopy one or
				// more times. (The exception is that when we're close to
				// exhausting
				...
				// current iteration has determined that a call to EmitLiteral
				// will
				// precede the next call to EmitCopy (if any).
				//
				// Step 1: Scan forward in the input looking for a 4-byte-long
				// match.
				// If we get close to exhausting the input exit and emit a final
				// literal.
				//
				// Heuristic match skipping: If 32 bytes are scanned with no
				// matches
				// found, start looking only at every other byte. If 32 more
				// bytes are
				...
				// more
				// bookkeeping, but for non-compressible data (such as JPEG)
				// it's a huge
				// win since the compressor quickly "realizes" the data is
				// incompressible
				// and doesn't bother looking for matches everywhere.
				//
				// The "skip" variable keeps track of how many bytes there are
				// since the
				// last match; dividing it by 32 (ie. right-shifting by five)
				// gives the
				...
				// Step 2: A 4-byte match has been found. We'll later see if
				// more
				// than 4 bytes match. But, prior to the match, input
				// bytes [nextEmit, ip) are unmatched. Emit them as "literal
				// bytes."
				...
				// input immediately after what was consumed by the last
				// EmitCopy call.
				//
				// If we exit this loop normally then we need to call
				// EmitLiteral next,
				// though we don't yet know how big the literal will be. We
				// handle that
				// by proceeding to the next iteration of the main loop. We also
				// can exit
	...
	private static int[] findCandidate(byte[] input, int ipIndex, int ipLimit, int inputOffset, int shift,
		...
		for (ipIndex += 1; ipIndex
				+ bytesBetweenHashLookups(skip) <= ipLimit; ipIndex += bytesBetweenHashLookups(skip++)) {
			...
			// if the 4 byte sequence a the candidate index matches the sequence
			// at the
	...
	private static int[] emitCopies(byte[] input, final int inputOffset, final int inputSize, int ipIndex,
			byte[] output, int outputIndex, short[] table, int shift, int candidateIndex) {
		// Step 3: Call EmitCopy, and then see if another EmitCopy could
		// be our next move. Repeat until we find no match for the
	...
	private static int findMatchLength(byte[] s1, int s1Index, byte[] s2, final int s2Index, int s2Limit) {
		assert (s2Limit >= s2Index);

		if (SnappyInternalUtils.HAS_UNSAFE) {
			int matched = 0;

			while (s2Index + matched <= s2Limit - 4 && SnappyInternalUtils.loadInt(s2,
					s2Index + matched) == SnappyInternalUtils.loadInt(s1, s1Index + matched)) {
				matched += 4;
			}
	...
	private static int getHashTableSize(int inputSize) {
		...
		// }
		//
		// assert 0 == (newHashTableSize & (newHashTableSize - 1)) : "hash must
		// be power of two";
		// assert newHashTableSize <= kMaxHashTableSize : "hash table too
		// large";
