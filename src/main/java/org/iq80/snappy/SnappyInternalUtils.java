...
final class SnappyInternalUtils {
	...
	static {
		// Try to only load one implementation of Memory to assure the call
		// sites are monomorphic (fast)
		Memory memoryInstance = null;

		// TODO enable UnsafeMemory on big endian machines
		//
		// The current UnsafeMemory code assumes the machine is little endian,
		// and will
		// not work correctly on big endian CPUs. For now, we will disable
		// UnsafeMemory on
		// big endian machines. This will make the code significantly slower on
		// big endian.
		// In the future someone should add the necessary flip bytes calls to
		// make this
...
//						.loadClass("org.iq80.snappy.SlowMemory").asSubclass(Memory.class);
//				Memory slowMemory = slowMemoryClass.newInstance();
	...
	/**
	 * Reads <i>length</i> bytes from <i>source</i> into <i>dest</i> starting at
	 * <i>offset</i>. <br/>
	 * <p/>
	 * The only case where the <i>length</i> <tt>byte</tt>s will not be read is
	 * if <i>source</i> returns EOF.
	 *
	 * @param source
	 *            The source of bytes to read from. Must not be
	 *            <code>null</code>.
	 * @param dest
	 *            The <tt>byte[]</tt> to read bytes into. Must not be
	 *            <code>null</code>.
	 * @param offset
	 *            The index in <i>dest</i> to start filling.
	 * @param length
	 *            The number of bytes to read.
	 * @return Total number of bytes actually read.
	 * @throws IndexOutOfBoundsException
	 *             if <i>offset</i> or <i>length</i> are invalid.
	 */
	...
	static int skip(InputStream source, int skip) throws IOException {
		// optimization also avoids potential for error with some implementation
		// of
		// InputStream.skip() which throw exceptions with negative numbers (ie.
		// ZipInputStream).
		if (skip <= 0) {
			return 0;
		}
